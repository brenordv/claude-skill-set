...
repeatable `--exclude <glob>` adjust the coverage policy (`tests/` paths, `test_*.py`,
`*_test.py`, and `conftest.py` are excluded by default). Coverage sources come from the target
